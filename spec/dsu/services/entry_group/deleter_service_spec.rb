...
RSpec.describe Dsu::Services::EntryGroup::DeleterService do
  ...
  describe '#call' do
    ...
    context 'when there are entry groups for the times given' do
      ...
      it do
        expect(Dsu::Services::EntryGroup::CounterService.new(times: times).call).to be times.count
      end

      it 'deletes the entry groups' do
        service.call
        expect(Dsu::Services::EntryGroup::CounterService.new(times: times).call).to be_zero
      end

      it 'returns the deleted the entry group count' do
